import java.io.*;
import java.util.*;

public class Main {
	static int N, M, K;
	static long[] numbers, tree;

	public static void main(String[] args) throws Exception {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
		StringTokenizer st = new StringTokenizer(br.readLine());
		N = Integer.parseInt(st.nextToken());
		M = Integer.parseInt(st.nextToken());
		K = Integer.parseInt(st.nextToken());
		numbers = new long[N + 1];
		tree = new long[N * 4];

		for (int idx = 1; idx <= N; idx++) {
			numbers[idx] = Integer.parseInt(br.readLine());
		}

		init(1, N, 1);

		for (int idx = 0; idx < M + K; idx++) {
			st = new StringTokenizer(br.readLine());
			if (st.nextToken().equals("1")) {
				int changeIndex = Integer.parseInt(st.nextToken());
				int changeValue = Integer.parseInt(st.nextToken());
				update(changeIndex, changeValue, 1, 1, N);
			} else {
				int left = Integer.parseInt(st.nextToken());
				int right = Integer.parseInt(st.nextToken());
				bw.write(cal(left, right, 1, 1, N) + "\n");
			}
		}

		bw.flush();
		br.close();
		bw.close();
	}

	static long init(int left, int right, int node) {
		if (left == right) {
			return tree[node] = numbers[left];
		}
		int mid = (left + right) / 2;
		long leftValue = init(left, mid, node * 2);
		long rightValue = init(mid + 1, right, node * 2 + 1);
		return tree[node] = (leftValue * rightValue) % 1_000_000_007;
	}

	static long update(int index, int newValue, int node, int nodeLeft, int nodeRight) {
		if (nodeRight < index || index < nodeLeft) {
			return tree[node];
		}
		if (nodeLeft == nodeRight) {
			return tree[node] = newValue;
		}
		int mid = nodeLeft + (nodeRight - nodeLeft) / 2;
		long leftValue = update(index, newValue, node * 2, nodeLeft, mid);
		long rightValue = update(index, newValue, node * 2 + 1, mid + 1, nodeRight);
		return tree[node] = (leftValue * rightValue) % 1_000_000_007;
	}

	static long cal(int left, int right, int node, int nodeLeft, int nodeRight) {
		if (nodeRight < left || right < nodeLeft) {
			return 1;
		}
		if (left <= nodeLeft && nodeRight <= right) {
			return tree[node];
		}
		int mid = nodeLeft + (nodeRight - nodeLeft) / 2;
		long leftValue = cal(left, right, node * 2, nodeLeft, mid);
		long rightValue = cal(left, right, node * 2 + 1, mid + 1, nodeRight);
		return (leftValue * rightValue) % 1_000_000_007;
	}
}

